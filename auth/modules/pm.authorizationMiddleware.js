...
function isPMapiInvalid(req) {
    ...
    if (authUtil.compareUrl("/api/a/pm/flow", req.path) || authUtil.compareUrl("/api/a/pm/flow/count", req.path)  || authUtil.compareUrl("/api/a/pm/flow/status/count", req.path)) {
        if (req.method === "GET") {
    ...
    if (authUtil.compareUrl("/api/a/pm/flow/{id}/deploy", req.path)) {
        let permissionApp = [];
    ...
    if (authUtil.compareUrl("/api/a/pm/flow/{app}/startAll", req.path) || authUtil.compareUrl("/api/a/pm/flow/{app}/stopAll", req.path)) {
        let app = pathSegment[5];
        return !(appsAdmin.indexOf(app) > -1);
    }
    if (authUtil.compareUrl("/api/a/pm/flow/{id}/stop", req.path) || authUtil.compareUrl("/api/a/pm/flow/{id}/start", req.path)) {
        let permissionApp = [];
        let allowedPermission = [];
        let appAdminFlag = appsAdmin.indexOf(req.apiDetails.app) > -1;
        if (appAdminFlag) return false;
    ...
    if (authUtil.compareUrl("/api/a/pm/flow/{id}", req.path) || authUtil.compareUrl("/api/a/pm/flow/{id}/{action}", req.path)) {
        let permissionApp = [];
        let allowedPermission = [];
        let appAdminFlag = appsAdmin.indexOf(req.apiDetails.app) > -1;
        if (appAdminFlag) return false;
    ...
    if (authUtil.compareUrl("/api/a/pm/dataFormat", req.path) || authUtil.compareUrl("/api/a/pm/dataFormat/count", req.path)) {
        if (req.method === "GET") {
    ...
    if (authUtil.compareUrl("/api/a/pm/dataFormat/{id}", req.path)) {
        let permissionApp = [];
        let allowedPermission = [];
        let appAdminFlag = appsAdmin.indexOf(req.apiDetails.app) > -1;
        if (appAdminFlag) return false;
    ...
    if (authUtil.compareUrl("/api/a/pm/agentRegistry", req.path) || authUtil.compareUrl("/api/a/pm/agentRegistry/count", req.path)) {
        if (req.method === "GET") {
    ...
    if (authUtil.compareUrl("/api/a/pm/agentMonitoring", req.path) || authUtil.compareUrl("/api/a/pm/agentMonitoring/count", req.path)) {
        if (req.method === "GET") {
            let permissionApp = req.user.roles.filter(_r => (_r.id === "PMABC" || _r.id === "PMABU" || _r.id === "PVAB") && _r.entity == "AGENT").map(_r => _r.app);
            let manageGroupApps = req.user.roles.filter(_r => (_r.id === "PMGAP" || _r.id === "PVGAP") && _r.entity === "GROUP").map(_r => _r.app);
            let exceptionAgent = req.user.roles.filter(_r => (_r.id === "PMABC" || _r.id === "PMABU" || _r.id === "PVAB") && _r.entity.startsWith("AGENT_")).map(_r => _r.entity.substr(6));
    ...
    if (authUtil.compareUrl("/api/a/pm/agentRegistry/{id}/password", req.path)) {
        let permissionApp = [];
        let allowedPermission = [];
        if (req.method === "GET") {
    ...
    if (authUtil.compareUrl("/api/a/pm/agentRegistry/{id}/enable", req.path) || authUtil.compareUrl("/api/a/pm/agentRegistry/{id}/disable", req.path)) {
        let permissionApp = [];
        let allowedPermission = [];
        if (req.method === "PUT") {
    ...
    if (authUtil.compareUrl("/api/a/pm/agentRegistry/{id}", req.path)) {
        let permissionApp = [];
        let allowedPermission = [];
        if (req.method === "GET") {
    ...
    if (authUtil.compareUrl("/api/a/pm/{app}/interaction", req.path) || authUtil.compareUrl("/api/a/pm/{app}/interaction/count", req.path) || authUtil.compareUrl("/api/a/pm/{app}/interactionBlock", req.path) || authUtil.compareUrl("/api/a/pm/{app}/interactionBlock/count", req.path)) {
        if (req.method === "GET") {
    ...
    if (authUtil.compareUrl("/api/a/pm/{app}/download/{agentType}/{id}/{type}", req.path) || authUtil.compareUrl("/api/a/pm/{app}/interaction/redownloadFile", req.path)) {
        if (authUtil.compareUrl("/api/a/pm/{app}/download/{agentType}/{id}/{type}", req.path)) {
    ...
    if (authUtil.compareUrl("/api/a/pm/ieg/download/{type}", req.path) || authUtil.compareUrl("/api/a/pm/agentRegistry/IEG/password", req.path)) {
        return true;
    }
    if (authUtil.compareUrl("/api/a/pm/flow/{app}/startAll", req.path) || authUtil.compareUrl("/api/a/pm/flow/{app}/stopAll", req.path)) {
        return appsAdmin.indexOf(pathSegment[5]) === -1;
    }
    if (authUtil.compareUrl("/api/a/pm/{app}/partner/{partnerid}/startAll", req.path) || authUtil.compareUrl("/api/a/pm/{app}/partner/{partnerid}/stopAll", req.path)) {
